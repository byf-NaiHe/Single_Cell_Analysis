...
dir.startrac <- "../data/tcr/startrac/CD8"
...

{
    ...
    if(!file.exists(sprintf("%s/CD8.out.startrac.nperm1000.rds",dir.startrac))){
        tic("Startrac.run")
        out <- Startrac.run(in.dat[stype=="CD8",], proj="panC",verbose=F,cores=6,n.perm=1000)
        toc()
        saveRDS(out,sprintf("%s/CD8.out.startrac.nperm1000.rds",dir.startrac))
    }
    out <- readRDS(sprintf("%s/CD8.out.startrac.nperm1000.rds",dir.startrac))
...
{
    if(!file.exists(sprintf("%s/CD8.out.only%s.startrac.nperm1000.rds",dir.startrac,a.loc))){
        tic("Startrac.run")
        in.dat.flt <- in.dat[stype=="CD8" & loc==a.loc,]
        ...
        toc()
        saveRDS(out.onlyALoc,sprintf("%s/CD8.out.only%s.startrac.nperm1000.rds",dir.startrac,a.loc))
    }
}
...
############
cancerType.vec <- in.dat[stype=="CD8",unique(cancerType)]

if(!file.exists(sprintf("%s/CD8.out.startrac.byCancerType.rds",dir.startrac)))
{
    res.byCancerType <- llply(cancerType.vec,function(x){
                      Startrac.run(in.dat[stype=="CD8" & cancerType==x,],
                           proj=x,verbose=F,cores=4,n.perm=NULL)
                            })
    names(res.byCancerType) <- cancerType.vec
    saveRDS(res.byCancerType,sprintf("%s/CD8.out.startrac.byCancerType.rds",dir.startrac))
}

if(!file.exists(sprintf("%s/CD8.out.startrac.byCancerType.onlyT.rds",dir.startrac)))
{
    res.byCancerType.onlyT <- llply(cancerType.vec,function(x){
                        Startrac.run(in.dat[stype=="CD8" & cancerType==x & loc=="T",],
                             proj=x,verbose=F,cores=4,n.perm=NULL)
                            })
    names(res.byCancerType.onlyT) <- cancerType.vec
    saveRDS(res.byCancerType.onlyT,sprintf("%s/CD8.out.startrac.byCancerType.onlyT.rds",dir.startrac))
}
############

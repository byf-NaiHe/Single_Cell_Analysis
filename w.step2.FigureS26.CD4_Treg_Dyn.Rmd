...
if (T){
# diffmap
diffmap = fread(sprintf("%s/../../data/metaInfo/%s_Treg.diffMap.txt.gz", oDir,stype), header=T, stringsAsFactors=F, sep="\t")
diffmap = diffmap[,c("V1","1","2")] %>% as.data.frame() %>% tibble::column_to_rownames("V1")
colnames(diffmap) = c("DC1","DC2")

# dpt_pseudotime
dpt = fread(sprintf("%s/../../data/metaInfo/%s_Treg.dpt_pseudotime.txt.gz", oDir,stype), header=T, stringsAsFactors=F, sep="\t")
...
#
saveRDS(sce, file=sprintf("%s/%s_Treg.sce.add_info.rds",oDir,stype))
}

####
sce = readRDS(sprintf("%s/%s_Treg.sce.add_info.rds",oDir,stype))
```
...
dat$meta.cluster = factor(as.character(dat$meta.cluster), levels=rev(c("CD4.c18.Treg.RTKN2","CD4.c19.Treg.S1PR1","CD4.c21.Treg.OAS1","CD4.c20.Treg.TNFRSF9")))
p = ggplot(dat, aes(x=meta.cluster, color=meta.cluster, y=dpt_order_perc)) + 
      geom_boxplot(outlier.shape=NA) +
      geom_point_rast(alpha=0.5, size=0.75, shape=16, raster.dpi=300, 
      ...
      coord_flip()
print(p)
ggsave(p, file=sprintf("%s/%s_Treg.dpt_pseudotime.boxplot.pdf",oDir,stype), width=4,height=2.5)
```
...
if (T){
  ...
  saveRDS(gam.test, file=sprintf("%s/%s_Treg.dyn_genes.rds",oDir,stype))
}else{
  gam.test = readRDS(sprintf("%s/%s_Treg.dyn_genes.rds",oDir,stype))
}
```
...
highlight.genes = c("FOXP3","CTLA4","IL2RA","TNFRSF9","BATF","IKZF2", "HIVEP1",
                    "ZFP36L2","RARRES3","CXCR4","KLF2",
                    "IKZF4","ZBTB32","IRF4","CCL22","IL1R1",
                    "CLTA","LAG3","IL1RN","XRCC5")
#
...
hm.obj = heatmap_sm(sce[plot.gene,], assay.name="exprs",
                 colSet=colSet,
                 out.prefix=sprintf("%s/%s_Treg.dyn_genes.heatmap", oDir, stype),
                 columns=c("dpt_order_perc","cluster.name"), 
                 ...
                 pdf.width=11.5, pdf.height=9, do.scale=F,
                 ...
                 #row_split=4
                 row_km=3, row_km_repeats=100, #row_title=NULL
                 )
print(hm.obj)
```
...
genes = c("FOXP3","BATF","HIVEP1")
#
for(gene in genes){
  p = plotONEgene(gene, sce, colSet)
  print(p)
  ggsave(p, file=sprintf("%s/%s_Treg.dyn_genes.%s.pdf", oDir, stype, gene), width=7.5,height=3)
}
```

...
colData(sce) <- DataFrame(meta.tb,row.names=meta.tb$cellID)
...
rowData(sce.mini) <- rowData(sce)[,c("geneID","geneSymbol","display.name","seu.id")]
...
sceToLoom(sce.dump,
		  ...
		  rd.map,
		  minCountsPerGene=0,minSamples=0)
